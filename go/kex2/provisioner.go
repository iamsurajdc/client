...
func (p *provisioner) pickFirstConnection() (err error) {
	...
	if len(p.arg.Secret) != 0 {
		...
		xp = rpc.NewTransport(conn, p.arg.Provisioner.GetLogFactory(), nil, maxFrameLength)
	...
	case sec := <-p.arg.SecretChannel:
		...
		p.xp = rpc.NewTransport(p.conn, p.arg.Provisioner.GetLogFactory(), nil, maxFrameLength)
	case <-p.arg.Ctx.Done():
		err = ErrCanceled
